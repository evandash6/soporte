...
@section('title', translate('Tickets', 'tickets'))
@section('content')
            ...
            <div class="counter">
                <div class="card-v">
                    ...
                    <div class="counter-meta">
                        <h3 class="mb-2">{{ translate('Opened Tickets', 'tickets') }}</h3>
        ...
        <div class="col">
            <div class="counter counter-danger">
                <div class="card-v">
                    ...
                    <div class="counter-meta">
                        <h3 class="mb-2">{{ translate('Closed Tickets', 'tickets') }}</h3>
        ...
        <div class="row row-cols-1 g-4">
            <div class="col">
                <div class="row row-cols-auto justify-content-between align-items-center g-2">
                    <div class="col">
                        <h4 class="mb-0">{{ translate('All Tickets', 'tickets') }}</h4>
                    </div>
                    <div class="col">
                        @if (request('search') || request('status_opened') || request('status_closed'))
                            <a href="{{ route('user.tickets.index') }}" class="btn btn-outline-danger btn-md me-2"><i
                                    class="fa-solid fa-filter-circle-xmark me-2"></i>{{ translate('Clear', 'tickets') }}</a>
                        @endif
                        <button class="btn btn-outline-primary btn-md" data-bs-toggle="modal"
                            data-bs-target="#ticketsFilter">
                            <i class="fa fa-filter me-2"></i>
                            {{ translate('Filter', 'tickets') }}
                        </button>
                    </div>
                </div>
            </div>
                ...
                <div class="dash-table d-none d-lg-block">
                    <div class="table-responsive-xl">
                        <table class="table">
                            <thead>
                                <tr>
                                    <th>{{ translate('ID', 'tickets') }}</th>
                                    <th class="text-start">{{ translate('Subject', 'tickets') }}</th>
                                    <th class="text-center">{{ translate('Priority', 'tickets') }}</th>
                                    <th class="text-center">{{ translate('Status', 'tickets') }}</th>
                                    <th>{{ translate('Created Date', 'tickets') }}</th>
                            ...
                            <tbody class="align-middle">
                                @forelse ($tickets as $ticket)
                                    <tr>
                                        <td>
                                            <a
                                                href="{{ route('user.tickets.show', $ticket->id) }}">#{{ $ticket->id }}</a>
                                        </td>
                                        <td class="text-start">
                                        ...
                                        <td class="text-center">
                                            @if ($ticket->isOpened())
                                                <span class="badge bg-primary">{{ translate('Opened', 'tickets') }}</span>
                                            @else
                                                <span class="badge bg-danger">{{ translate('Closed', 'tickets') }}</span>
                                            @endif
                                        </td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="text-center">
                                            <p class="mb-0 p-3">{{ translate('No tickets found', 'tickets') }}</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content p-4">
                <div class="modal-header border-0 p-0 mb-4">
                    <h5 class="modal-title" id="exampleModalLabel">{{ translate('Tickets Filter', 'tickets') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body p-0">
                    <form id="filterForm" action="{{ route('user.tickets.index') }}" method="GET">
                        <div class="row row-cols-1 g-3">
                            <div class="col">
                                <div class="form-search">
                                    <input type="text" name="search" class="form-control form-control-md"
                                        placeholder="{{ translate('Type to Search...', 'tickets') }}"
                            ...
                            <div class="col">
                                <label class="form-label">{{ translate('Tickets Status', 'tickets') }}</label>
                                    ...
                                    <div class="col">
                                        <div class="form-check">
                                            ...
                                            <label class="form-check-label"
                                                for="ticketStatus1">{{ translate('Opened Tickets', 'tickets') }}</label>
                                        </div>
                                    </div>
                                    <div class="col">
                                        <div class="form-check">
                                            ...
                                            <label class="form-check-label"
                                                for="ticketStatus2">{{ translate('Closed Tickets', 'tickets') }}</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="modal-footer border-0 p-0 mt-4">
                    <button type="button" class="btn btn-outline-danger"
                        data-bs-dismiss="modal">{{ translate('Close', 'tickets') }}</button>
                    <button form="filterForm" class="btn btn-primary">{{ translate('Filter', 'tickets') }}</button>
                </div>
            </div>
        </div>
    </div>
